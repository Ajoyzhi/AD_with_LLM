...
class VisionLLMAgent(autonomous_agent.AutonomousAgent):
    ...
    def run_step(self, input_data, timestamp):
        ...
        concat_image = np.vstack(images_list)  # [1200, 400, 3]
        self.visual_feature_buffer.append(concat_image)

        # output the all the buffered image, but only return the newset image in the buffer
        def save_numpy_as_image():
            path = "./carla_outputs/vision_llm_camera/"
            if not os.path.exists(path):
                os.makedirs(path)

            for id, image_np in enumerate(self.visual_feature_buffer):
                if id < self.image_id:
                    continue
                else:
                    # to avoid saving again
                    ...
                    self.image_id += 1
                    save_path = path + "concat_image" + str(self.change_instruction) + "_" + str(self.image_id) + ".png"
                    cv2.imwrite(save_path, image_np)
            save_path = path + "concat_image" + str(self.change_instruction) + "_" + str(self.image_id) + ".png"
            return save_path

        image_path = save_numpy_as_image()
        ...
        with torch.cuda.amp.autocast(enabled=True):
            ...
            prompt = (
                ...
                f"The concatenation image is from 4 cameras in your car, and the order from top to bottom is front, left, right and rear. "
                f"The current instruction you receive is {self.curr_instruction}. "
                f"The things you should notice is {self.curr_notice}. "
                f"Your current speed is {velocity.item()}, and target point is {input_data['target_point'].item()}."
                f"Please analyze the surroundings of the vehicle, and generate 5 waypoints."
                f"Then give a float number to show stop probability whether the vehicle should stop. "
