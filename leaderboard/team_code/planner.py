...
class InstructionPlanner(object):
    def __init__(self, notice_light_switch = False):
        self._vehicle = CarlaDataProvider.get_hero_actor()
        self._world = self._vehicle.get_world()
        lights_list = self._world.get_actors().filter("*traffic_light*")
        self._map = self._world.get_map() # Map(name=Carla/Maps/Town12/Town12)
        ...
        last_part = str(self._map).split('/')[-1].rstrip(')')
        self.town_id = last_part.split(')')[0]  # Town12
        ...
        match = re.search(r'\d+', self.town_id)
        seed = int(match.group()) if match else None
        random.seed(seed)
    ...
    def command2instruct(self, tick_data, routes=None, dis_on=True):
    ...
    def command2mislead(self, tick_data):
        town_id = self.town_id
        mislead_id = None
        mislead_id_list = []
        command = tick_data["next_command"]
...
class MultiInsturctionsPlanner(InstructionPlanner):
    def __init__(self, global_plan, town_id, notice_light_switch = False):
        super().__init__(town_id, notice_light_switch)
